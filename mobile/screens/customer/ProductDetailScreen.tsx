 ...
 * Uses `useAsyncWithTimeout` to manage loading/error state.
 */
const ProductDetailScreen: React.FC = () => {
  ...
  const fetchFn = useCallback(
    () => (getProductById ? getProductById(productId) : Promise.resolve({ data: null, error: { message: 'Hook not ready', isFatal: false } })),
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.contentCard}>
          ...
          <View style={styles.badgesRow}>
            ...
            {product.stock <= 3 && product.stock > 0 && <DealBadge variant="lastChance" label={`${product.stock} left`} />}
          ...
          <View style={styles.infoRow}>
            <Text style={styles.infoIcon}>📦</Text>
            <Text style={styles.infoText}>{product.stock} portion{product.stock !== 1 ? 's' : ''} left</Text>
          </View>
          ...
          <View style={styles.priceSection}>
            ...
            <View style={styles.savingsBadge}>
              <Text style={styles.savingsText}>Save ${((product.original_price_cents - product.discounted_price_cents) / 100).toFixed(2)}</Text>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  qtyValue: { fontSize: FontSize.xl, fontWeight: '800', color: Colors.charcoal, minWidth: 24, textAlign: 'center' },
  stickyFooter: {
    position: 'absolute',
    bottom: 0,
    left: 0,
