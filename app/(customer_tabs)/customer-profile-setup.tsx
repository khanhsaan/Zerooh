 ...
 *
 * Onboarding screen for new customer users. Collects first and last name,
 * then calls `useProfile.setUpCustomerProfile` to write to `user_profiles`.
 * On success, the user stays in the customer tabs and can continue exploring.
 */
export default function CustomerProfileSetupScreen() {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const result = await setupProfile({ firstName: firstName.trim(), lastName: lastName.trim() });
    setLoading(false);
    if (result.error) {
